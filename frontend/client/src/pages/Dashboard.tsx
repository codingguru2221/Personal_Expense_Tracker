...
import { useUser, useAccounts, useTransactions } from "@/hooks/use-finance";
...
const categoryData = [
  { name: 'Shopping', value: 400, color: '#8884d8' },
  { name: 'Food', value: 300, color: '#82ca9d' },
  { name: 'Transport', value: 300, color: '#ffc658' },
  { name: 'Bills', value: 200, color: '#ff8042' },
];

export default function Dashboard() {
  const { data: user } = useUser(1); // Mock user ID 1
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-card rounded-3xl p-6 shadow-sm border border-border/50">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-bold font-display">Spending Analytics</h3>
              ...
              <div className="mt-4 space-y-3">
                 {categoryData.map((cat) => (
                    <div key={cat.name} className="flex justify-between items-center text-sm">
                       ...
                       <span className="font-semibold">{Math.round((cat.value / 1200) * 100)}%</span>
                    </div>
                 ))}
              </div>
            </div>
